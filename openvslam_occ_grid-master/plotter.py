	...
	def local_target_select_and_calculate_steering(self): 
		...
		if (int(self.current_heading_x - self.current_local_x) != 0): 
			...
			if(Slope != 0):
				...
				if((local_area_point1_x < station_x < local_area_point2_x) and (local_area_point1_y < station_y < local_area_point2_y)):
					# 영역안에 station이 있을 경우 follow station!
					current_heading_vector = [self.current_heading_x - self.current_local_x,self.current_heading_y - self.current_local_y]
					theta = theta_between_two_vector(Direction_vector,current_heading_vector)
					# print(theta) # 플러스면 우회전 마이너스면 좌회전
					if(theta<-20):
						theta = -20
					elif (theta > 20):
						theta = 20
					print(distance_between_current_local_and_station)
					if(distance_between_current_local_and_station<5):
						print("stop")
						self.car_msg.drive.speed = 0
					else:
						self.car_msg.drive.speed = 0.3
					self.car_msg.drive.steering_angle = -(theta*math.pi/180) # 부호 반대임, radian으로 넣어준다
					print(self.car_msg.drive.steering_angle)
					...
					if(most_minumun_differnece_index_of_keyframe>-1):
						...
						theta = theta_between_two_vector(Direction_vector,keyframe_heading_vector)
						# print(theta) # 플러스면 우회전 마이너스면 좌회전
						if(theta<-20):
							theta = -20
						elif (theta > 20):
