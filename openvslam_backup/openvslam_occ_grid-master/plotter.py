...
class Plotter:
	def __init__(self, file_name, config_file_name):
		...
		self.pub = rospy.Publisher('/ackermann_steerimg',Float32)

	def get_pose_and_heading_of_current_frame(self, data):
		...
		for index in range(len(self.keyframes_pos)):
			cv2.circle(self.img,(int(-self.keyframes_pos[index][0]*self.scale_integer+self.image_row/2),
							int(self.keyframes_pos[index][1]*self.scale_integer+self.image_cols/10)),1,(255,255,255))
		...
		if(self.stations_pos!=0):
			for index in range(len(self.stations_pos)):
				cv2.circle(self.img,(int(-self.stations_pos[index][0]*self.scale_integer+self.image_row/2),
								int(self.stations_pos[index][1]*self.scale_integer+self.image_cols/10)),3,(255,255,0),-1)

		# draw current_position
		self.current_local_x = int(-self.current_location_pos[0]*self.scale_integer+self.image_row/2)
		self.current_local_y = int(self.current_location_pos[1]*self.scale_integer+self.image_cols/10)
		...
		self.current_heading_y = int((self.current_location_pos[1]*self.scale_integer + self.current_location_heading_vector[1]*self.scale_integer+self.image_cols/10))
	...
	def local_target_select_and_calculate_steering(self): 
		...
		if(Slope != 0):
			...
			# local_area_point1 & local_area_point1가 영역 범위임
			...
			station_y = (self.stations_pos[0][1]*self.scale_integer+self.image_cols/10)
			most_minumun_differnece_index_of_keyframe = -1
			Difference = 10000
			Direction_vector = [station_x - self.current_local_x,station_y - self.current_local_y]

			# secs = time.time()
			# print(secs)

			distance_between_heading_and_station = (self.current_heading_x - station_x)**2 + (self.current_heading_y - station_y)**2
			...
			for index in range(len(self.keyframes_pos)):
				...
				keyframe_y = (self.keyframes_pos[index][1]*self.scale_integer+self.image_cols/10)
				distance_between_keyframe_and_station = (keyframe_x - station_x)**2 + (keyframe_y - station_y)**2
				# 관심영역 안에 keyframes_pos[index]가 존재하면
			...
			if(most_minumun_differnece_index_of_keyframe>-1):
				...
				keyframe_y = int(self.keyframes_pos[most_minumun_differnece_index_of_keyframe][1]*self.scale_integer+self.image_cols/10)
				cv2.circle(self.img,(keyframe_x,keyframe_y),5,(255,255,255),-1)

				keyframe_heading_vector =  [keyframe_x - self.current_local_x,keyframe_y - self.current_local_y]

				theta = theta_between_two_vector(Direction_vector,keyframe_heading_vector)*(180/math.pi)
				print(theta) # 마이너스면 좌회전 플러스면 우회전
				self.pub.publish(int(theta*math.pi/180))
